...
.getDbLooCovariateData <- function(connection,
  ...
  sql <- paste("SELECT @row_id_field AS row_id, 1 AS covariate_id,",
    ...
    "AS covariate_value", "FROM @cohort_table c", "INNER JOIN @cdm_database_schema.observation_period op",
    "ON op.person_id = c.subject_id", "WHERE cohort_start_date >= observation_period_start_date",
    "AND cohort_start_date <= observation_period_end_date", "{@cohort_id != -1} ? {AND cohort_definition_id = @cohort_id}")
  ...
  analysisRef <- data.frame(analysisId = 1,
    ...
    startDay = 0, endDay = 0, isBinary = "N", missingMeansZero = "Y")
  # Construct analysis reference:
