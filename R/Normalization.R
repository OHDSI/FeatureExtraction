...
tidyCovariateData <- function(covariateData,
  ...
  if (covariateData$covariates %>%
    ...
    if (removeRedundancy) {
      ...
      if (covariateData$binaryCovariateIds %>%
        count() %>%
        pull() != 0) {
        if (isTemporalCovariateData(covariateData)) {
          ...
          # First, find all single covariates that, for every timeId, appear in every row with the
          # same value
          ...
          Andromeda::appendToTable(covariateData$deleteCovariateTimeIds,
                                   valueCounts[!duplicated(valueCounts$analysisId),

          c("covariateId", "timeId")])
          ...
          count() %>%
          pull(), " redundant covariate ID - time ID combinations")
        } else {
          # Non-temporal
          ...
          deleteCovariateIds <- c(deleteCovariateIds,
                                  valueCounts$covariateId[!duplicated(valueCounts$analysisId)])
