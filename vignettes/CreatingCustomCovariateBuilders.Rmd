...
The target table arguments are passed to all covariate construction functions by `getDbCovariateData`. If your custom builder only returns a `CovariateData` object and does not support writing directly to target tables, it should still include these arguments in the function signature to avoid unused argument errors.
