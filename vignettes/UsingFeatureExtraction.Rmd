...
sql <- SqlRender::loadRenderTranslateSql(sqlFilename = "cohortsOfInterest.sql"),
...
`loadRenderTranslateSql` (which combines three functions earlier versions of SqlRender) reads the SQL from the file into memory, replaces the two parameter names with the actual values, then translates the SQL into the dialect appropriate for the DBMS we already specified in the `connectionDetails`. Next, we connect to the server, and submit the rendered and translated SQL.
...
Creating covariates can take considerable computing time, and it is probably a good idea to save them for future sessions. Because `covariateData` objects use `ff`, we cannot use R's regular save function. Instead, we'll have to use the `saveCovariateData()` function:
