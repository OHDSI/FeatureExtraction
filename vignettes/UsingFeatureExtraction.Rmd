...
If we want to know what these numbers mean, we can use the `covariateRef` object that is part of any `covariateData` object. Because `covariateRef`, like other parts of `covariateData`, uses the `Andromeda` package, it is advised to load the `Andromeda` package:
