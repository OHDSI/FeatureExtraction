...
1. **Remove infrequent covariates**: Oftentimes the majority of features have non-zero values for only one or a few subjects in the cohort. These features are unlikely to end up in any fitted model, but can increase the computational burden, and removing them could be good idea. By default, covariates appearing in less than .1% of the subjects are removed.
2. **Normalization**: scale all covariate values to a value between 0 and 1 (by dividing by the max value for each covariate).
