...
```{r echo=FALSE,message=FALSE}
if (file.exists(file.path(vignetteFolder, "covariatesPerPerson"))){
  cvs <- covariateData$covariates
  cvs$rowId <- ff::ff(1:nrow(cvs))
  cvs
}
```
The columns are defines as follow:
...
### Normalizing and removing redundancy
...
```
```{r echo=FALSE,message=FALSE}
if (file.exists(file.path(vignetteFolder, "deletedCovariateIds.rds"))){
...
if (file.exists(file.path(vignetteFolder, "deletedCovariateIds.rds"))){
  library(ffbase)
   ...
   cr <- as.ram(covariateData$covariateRef[covariateData$covariateRef$covariateId %in% deletedCovariateIds, ])
...
```{r eval=FALSE}
saveCovariateData(covariateData2, "aggregatedCovariates")
```
...
One aim supported by the `FeatureExtraction` package is to create a table that can be include in a paper, providing some overall characteristics of the study population. Since this is typically the first table in a paper we refer to such a table as 'table 1'. A default table 1 is available in the `FeatureExtraction` package:
```{r eval=FALSE}
...
\fontsize{6}{6}
\selectfont
```{r, set-options,comment=NA,echo=FALSE,message=FALSE}
if (file.exists(file.path(vignetteFolder, "aggregatedCovariates"))){
  options(width=200)
  result <- createTable1(covariateData2)
  print(result, row.names = FALSE, right = FALSE)
...
Here we based table 1 on a `covariateData` object containing all default covariates, even though only a small fraction were used in the table. If we only want to extract those covariates needed for the table, we can use the `createTable1CovariateSettings` function:
