...
SELECT 
	CAST(measurement_concept_id AS BIGINT) * 1000 + @analysis_id AS covariate_id,
{@temporal} ? {
    time_id,
}	
{@aggregated} ? {
...
{@has_included_covariate_ids} ? {		AND CAST(measurement_concept_id AS BIGINT) * 1000 + @analysis_id IN (SELECT covariate_id FROM #included_cov_by_id)}
) by_row_id
{@aggregated} ? {		
...
SELECT covariate_id,
{@temporal} ? {
	CONCAT('Measurement: ', concept_id, '-', concept_name) AS covariate_name,
} : {
	CONCAT('Measurement during day @start_day through @end_day days relative to index: ', concept_id, '-', concept_name) AS covariate_name,
}
